...
export class Range {
	constructor(selector, options) {
		...
		this.areaDifference =
			this.rangeStartData.volume_end - this.rangeData.volume_end

		this.rangeInputsValidate = {
			volume_start: input =>
				validatesInputs.rangeCountArea(
					input,
					this.rangeStartData.volume_end - this.areaDifference
				),
			volume_end: input =>
				validatesInputs.rangeCountArea(
					input,
			...
			price_start: input =>
				validatesInputs.rangeCount(input, this.rangeStartData.price_end),
			price_end: input =>
				validatesInputs.rangeCount(input, this.rangeStartData.price_end),
		}
	...
	init() {
		...
		this.ranges.forEach(range => {
			...
			const slider = noUiSlider.create(range, {
				start: [
					this.rangeStartData[`${nameRange}_start`],
					this.rangeStartData[`${nameRange}_end`],
				],
				connect: true,
				...
				values.length &&
					values.forEach((value, i) => {
						if (i === 0) {
							...
							value.innerHTML =
								+value.textContent - this.areaDifference + '<span>м³</span>'
						}
		...
		for (let key in this.sliders) {
			if (this.sliders.hasOwnProperty(key)) {
				...
				this.sliders[key].on('change', (values, handle, unencoded) =>
					this.rangeChange(values, handle, unencoded)
				)
			}
		}
	...
	rangeSlide({ values, handle, unencoded, noUiSlider }) {
		...
		const inputs = document.querySelectorAll(
			`${this.options.selectorInput}[data-type-range="${typeRange}"]`
		)
		...
		this.inputs.length &&
			this.inputs.forEach(input => {
				input.addEventListener('input', () => {
					...
					clearTimeout(this.timerRange)
					this.timerRange = setTimeout(() => {
						const [inputAnother] = Array.from(this.inputs).filter(
							_input =>
								typeRange === _input.getAttribute('data-type-range') &&
								_input !== input
						)
						...
						if (typeValue === 'start') {
							if (+input.value >= +inputAnother.value.replace(/\s/g, '')) {
								...
								outputInfo({
									msg:
										'Начальное значение не может быть больше или равно конечному',
									msg_type: 'warning',
								})
								return
							}
							...
							if (+input.value <= +inputAnother.value.replace(/\s/g, '')) {
								...
								outputInfo({
									msg:
										'Конечное значение не может быть меньше или равно начальному',
									msg_type: 'warning',
								})
								return
							}
	...
	updateInputValue(nameRange, values) {
		const inputs = document.querySelectorAll(
			`${this.options.selectorInput}[data-type-range="${nameRange}"]`
		)
	...
	setSlider(nameRange, values) {
		...
		if (nameRange === 'volume') {
			if (+firstValue == 1 && +lastValue == 1.5) {
				...
				this.sliders[nameRange].set([
					isFirstValue ? +firstValue + this.areaDifference : +firstValue,
					+lastValue + this.areaDifference,
				])
			}
		} else {
