...
import {
	markerContent,
	modalWarehouse,
	modalWarehouseCurrent,
	warehouseList,
} from './html.js'
async function getStyleMap() {
	try {
		const response = await fetch(
			`${window.app.directory}/assets/data/customization.json`
		)
...
const mapInit = async warehouses => {
	...
	const selectMap = new Select('.select-map', {
		inputHtml: `<svg class='icon icon-arrow-left'><use xlink:href='${window.app.directory}/assets/img/svg/sprite.svg#arrow-left'></use></svg>`,
	...
	const modalInfoAllWarehouses = yandexMap.querySelector(
	...
	const controlsVertical = yandexMap.querySelector(
		'.ymaps3x0--controls_vertical'
	)

	controlsVertical.insertAdjacentHTML(
		'beforeend',
	...
	<use xlink:href='${window.app.directory}/assets/img/svg/sprite.svg#yandex-marker'></use>
	...
	function handlerClickMarker(markerElement, warehouse) {
		if (markerElement.classList.contains('_selected')) {
			...
			markerElementSelected &&
				markerElementSelected.classList.remove('_selected')
	...
	function openModalWarehouse(warehouse) {
		...
		if (mediaQueryList.matches) {
			modalInformationWarehouse.modal.querySelector(
				'.modal__body'
			).innerHTML = modalWarehouseCurrent(warehouse)
	...
	warehouses.length &&
		warehouses.forEach(warehouse => {
			...
			warehousesListMap.insertAdjacentHTML(
				'beforeend',
			...
			markerElement.addEventListener('click', () =>
				handlerClickMarker(markerElement, warehouse)
			)
