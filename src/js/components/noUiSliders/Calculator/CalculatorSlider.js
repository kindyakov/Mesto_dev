...
class CalculatorSlider extends BaseSlider {
	...
	createHtmlTooltip({ target }) {
		...
		if (noUiHandle) {
			noUiHandle.insertAdjacentHTML(
				'beforeend',
				`<div class="range-tooltip">
        <span></span>
        <img src="${window.app.directory}/assets/img/svg/range-icon.svg" class="range-tooltip_img" alt="Иконка"/>
        <img src="${window.app.directory}/assets/img/svg/range-icon-2.png" class="range-tooltip_img-bg" alt="Иконка"/>
      </div>`
			)
		}

		if (noUiConnects) {
			noUiConnects.insertAdjacentHTML(
				'beforeend',
				`<span class="carving"><span>`
			)
		}
	}
