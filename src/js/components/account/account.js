...
class Account {
	...
	events() {
		if (!this.account) return
		this.accountTabs.options.onChange = (
			nexTabBtn,
			prevTabBtn,
			nextTabContent,
			prevTabContent
		) => this.changeTabs(nexTabBtn, nextTabContent)
		...
		this.account.addEventListener('click', e => {
			if (e.target.closest('.btn-complete-lease')) {
				...
				this.accountTabs.switchTabs(
					this.account.querySelector('.account-departures-btn')
				)
		...
		this.accountLink &&
			this.accountLink.addEventListener('click', e => {
				e.preventDefault()
				document.cookie =
	...
	async init() {
      ...
      <i>
				<svg class='icon icon-exit'>
					<use xlink:href='${window.app.directory}/assets/img/svg/sprite.svg#exit'></use>
				</svg>
			</i>
			<span>Выйти</span>`
		}
		...
		try {
			this.loader.enable()
			this.warehouses = await this.getSchemes()
	...
	async getProfile() {
		try {
			...
			if (roomId && num_monthes) {
				this.formNewAgreement.createAgreement({
					profile: user,
					roomId,
					num_monthes,
				})
				...
				this.accountTabs.tabsBtns.forEach(btn => {
					if (!btn.classList.contains('account-tabs-btn-change-password')) {
						...
						document
							...
							.classList.add('create-password')
					}
				})

				this.accountTabs.switchTabs(
					this.accountTabs.tabs.querySelector(
						'.account-tabs-btn-change-password'
					)
				)
			}
		} catch (error) {
		...
		try {
			...
			const response = await fetch(`${wpData.ajax_url}/`, {
				...
				body: new URLSearchParams({
					action: 'warehouse_schemes',
				}),
			})

			// const response = await fetch(`${location.origin}/assets/schemes.json`)
	...
	async initTabs(tabsBtnActive, tabsContentActive) {
		try {
			...
			const accountStoreroomsRooms = document.querySelector(
				'.account-storerooms-rooms'
			)
			const btnTabs0 = document.querySelector(
				'[data-tabs-btn="account-tabs-0"]'
			)
			const btnTabs1 = document.querySelector(
				'.account-tabs-btn[data-tabs-btn="account-tabs-1"]'
			)
			const btnTabs5 = document.querySelector(
				'[data-tabs-btn="account-tabs-5"]'
			)
			...
			if (rooms.length) {
				...
				if (isOneRented(rooms, 0.45)) {
					...
					this.accountTabs.switchTabs(
						this.accountTabs.tabs.querySelector('.account-tabs-btn-my-data')
					)
					return
				} else if (
					!isOneRented(rooms) &&
					!isOneRented(test_rooms, 0.25) &&
					!isOneRented(rooms, 0.75)
				) {
	...
	async changeTabs(tabsBtnActive, tabsContentActive) {
		try {
			...
			this.storeroomsScheme = document.querySelector(
			...
			if (tabsContentActive.classList.contains('account-assets-storage')) {
				this.accessStorage.render({
					accountTabs: this.accountTabs,
					clientTotalData,
				})
				...
				tabsContentActive.classList.contains('account-payment-methods')
			) {
