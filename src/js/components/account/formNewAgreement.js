...
class FormNewAgreement {
	constructor() {
		...
		this.loader = new Loader(
			this.modalPaymentsRoom.modal.querySelector('.modal__body')
		)

		this.urlParams = new URLSearchParams(window.location.search)

		this.cards = new Cards(
			this.modalPaymentsRoom.modal.querySelector(
				'.account-storerooms__agreement_form-payment'
			),
			{ isLinkCard: true }
		)

		this.form = this.modalPaymentsRoom.modal.querySelector(
			'.modal-payments-room__form'
		)
		this.btnPaymentModal = this.modalPaymentsRoom.modal.querySelector(
			'.btn-payment-modal'
		)
		...
		this.inputAutoPayments = this.form.querySelector(
			'input[name="auto-payments"]'
		)
	...
	events() {
		...
		this.modalPaymentsRoom.options.onOpen = e => {
			const btn = e.target?.closest(
				this.modalPaymentsRoom.options.modalBtnActive
			)
			...
			if (btn.classList.contains('btn-rent-room')) {
				location.href = `${location.origin}/rent-room?ids=${encodeURIComponent(
					JSON.stringify([roomId])
				)}`
				return
			}

			this.clientData.rooms = this.clientData.rooms.filter(
				(room, index, self) =>
			...
			if (!this.clientData.client.user_type) {
				...
				if (agrId) {
					const [currentAgreement] = this.clientData.agreements.filter(
						agreement => +agreement.agrid === +agrId
					)
			...
			if (this.clientData.agreements) {
				...
				if (agrId) {
					;[currentAgreement] = this.clientData.agreements.filter(
						agreement => agreement.agrid == agrId
					)
					currentRoom = this.clientData.rooms.filter(
						room => +room.room_id === +currentAgreement.room_ids[0]
					)
				} else {
					;[currentRoom] = this.clientData.rooms.filter(
						room => +room.room_id === +roomId
					)[currentAgreement] = this.clientData.agreements.filter(
						agreement => agreement.agrid == currentRoom.agrid
					)
				}
			...
			if (oldRoomId && newRoomId) {
				...
				const [currentRoom] = this.clientData.rooms.filter(
					room => room.agrid === agrId
				)
				const [currentAgreement] = this.clientData.agreements.filter(
					agreements => agreements.agrid == agrId
				)
				price = currentRoom.price

				if (currentAgreement.recurrent_payments) {
					this.modalPaymentsRoom.modal
				...
				const [currentRoom] = this.allRooms.filter(
					room => +room.room_id === +roomId
				)
				price = currentRoom.price
				this.typePayment = 'before'
		...
		this.inputMonth &&
			this.inputMonth.addEventListener('input', e => {
				const itemMonthActive = this.form.querySelector('.item-month._active')
				const itemMonthCurrent = this.form.querySelector(
				...
				if (value < 6) {
					...
					this.sumPriceDiscCells =
						this.priceRoom - ((this.priceRoom * 5) / 100).toFixed(0)
				} else if (value > 10) {
					this.sumPriceDiscCells =
						this.priceRoom - ((this.priceRoom * 10) / 100).toFixed(0)
				}
		...
		this.btnPaymentModal.addEventListener('click', e => {
			...
			if (this.clientData.client.user_type === 'f') {
				if (this.typePayment === 'replace') {
					...
					formData.set(
						'return_url',
						location.href + `?room_id=${this.roomId}&num_monthes=${this.month}`
					)
	...
	renderItemsMonth(priceSum) {
		this.itemsMonth.length &&
			this.itemsMonth.forEach(item => {
	...
	renderModal({ price, roomId }) {
		...
		if (this.agrId) {
			this.modalPaymentsRoom.modal
				.querySelector('.rent-room__tariffs')
				.classList.add('_none')
		} else {
			this.modalPaymentsRoom.modal
				.querySelector('.rent-room__tariffs')
		...
		if (this.clientData.client.user_type === 'f') {
			this.modalPaymentsRoom.modal
				.querySelector('.payment-method-accordion')
				.classList.add('_none')
		} else if (this.clientData.client.user_type === 'u') {
			this.modalPaymentsRoom.modal
				.querySelector('.payment-method-accordion')
	...
	createAgreement({ profile, roomId, num_monthes }) {
		...
		formData.set(
			'room_ids',
			JSON.stringify(Array.isArray(roomId) ? roomId : [roomId])
		)
