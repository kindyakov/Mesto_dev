...
<<<<<<< HEAD
  ...
  const currentRoomsNames = rooms.length
    ? rooms
      .map((room, i) => {
        if (room.agrid === agreement.agrid) {
          return 'Кладовка ' + room.room_name
        } else return ''
      })
  ...
  const currentRooms = rooms.length
    ? rooms.filter(room => room.agrid === agreement.agrid)
    : []
  ...
  <div class="account-storerooms__agreement_control agreement-accordion-control _my-accordion-control">
    <h2 class="agreement2__row__right-col__title">${agreement.agrid ? `Оферта ${agreement.agrid}` : ''
    }</h2>
    <span>${currentRoomsNames.length ? currentRoomsNames.join(' • ') : 'Нет ячеек'
    }</span>
    <svg class='icon icon-arrow-left'>
      <use xlink:href='${window.app.directory
    }/assets/img/svg/sprite.svg#arrow-left'></use>
=======
      return acc + +room.price;
    } else {
      return acc;
...
<<<<<<< HEAD
    <p>Стоимость: <span> ${totalPrice ? formattingPrice(totalPrice) : ''
    }</span></p>
    <p>Осталось дней: <span> ${agreement.days_left ? agreement.days_left : ''
    }</span></p>
    <p>Дата следующего платежа: <span>${agreement.next_payment_date
      ? formattingMonthDDMMYYYY(agreement.next_payment_date)
      : ''
    }</span></p>
    <p>Плановая дата окончания: <span> ${agreement.agrplanenddate
      ? formattingMonthDDMMYYYY(agreement.agrplanenddate)
      : ''
    }</span></p>
  </div>
  <!--<div class="account-storerooms__agreement_row">
    ...
    data-room-id="${currentRooms.length ? currentRooms[0].room_id : ''}">
      <img src="${window.app.directory}/assets/img/icons/pdf.png" alt="Иконка">
=======
...
<<<<<<< HEAD
    <button style="flex: 1 1 50%;" class="agreement2__row__right-col__btn account-storerooms__agreement_button-payment btn-payment btn-modal-payments-room-open button-2" data-agreement-id="${agreement.agrid ? agreement.agrid : ''
    }" data-months="${rooms[0]?.rent_period}" data-room-ids="${agreement.room_ids.length ? agreement.room_ids.join(',') : ''
    }"><span>Оплатить</span></button>
    <button style="flex: 1 1 50%;" class="agreement2__row__right-col__btn account-storerooms__agreement_button-payment btn-cancel-autopayments button-2" data-agreement-id="${agreement.agrid ? agreement.agrid : ''
    }"><span>Отменить автоплатежи</span></button> 
=======
...
export function roomsHtml(data, className = '') {
  return `
<<<<<<< HEAD
  <div class="agreement2__row__right-col__product account-storerooms__room room-account-storerooms room-accordion _my-accordion ${className}" data-room-id="${data.room_id &&
    data.room_id}">
    <div class="account-storerooms__room_control room-accordion-control _my-accordion-control">
      <h2 class="title-product">${data.room_name ? 'Кладовка ' + data.room_name + `<br><span style="font-size: 14px;">(${data.warehouse_name})</span>` : ''
    }</h2>
      <p>${data.volume && '<span>' + data.volume + ' м³</span>'}${data.floor &&
    '<span>' + data.floor + ' ярус</span>'}</p>
...
<<<<<<< HEAD
      <button class="btn-payment btn-modal-payments-room-open button-2" data-room-id="${data.room_id ? data.room_id : ''
    }" data-agreement-id="${data.agrid ? data.agrid : ''
    }"><span>Оплатить</span></button>
=======
...
<<<<<<< HEAD
  return `<div class="scheme-room scheme-room-old" data-room-id="${data.room_id ? data.room_id : ''
                ...
                <div class="scheme-room__top">
                  <h5>${data.room_name ? 'Кладовка ' + data.room_name : ''}</h5>
=======
  return `<div class="scheme-room scheme-room-old" data-room-id="${data.room_id ? data.room_id : ''}">
                <div class="scheme-room__top">
...
<<<<<<< HEAD
                  <button class="btn-complete-lease button" data-room-id="${data.room_id ? data.room_id : ''
    }"><span>Завершить аренду</span></button>
                  <button class="btn-payment button btn-modal-payments-room-open" data-room-id="${data.room_id ? data.room_id : ''
    }" data-agreement-id="${data.agrid ? data.agrid : ''
    }" data-months="${data.rent_period ? data.rent_period : ''
    }"><span>Оплатить</span></button>
                  <!--<button class="account-storerooms__agreement_button-download btn-download-agreement" data-room-id="${data.room_id ? data.room_id : ''
    }">
                    <img src="${window.app.directory
    }/assets/img/icons/pdf.png" alt="Иконка">
=======
...
<<<<<<< HEAD
  return `<div class="scheme-room scheme-room-new" data-room-id="${data.room_id &&
    data.room_id}">
                <div class="scheme-room__top">
                  <h5>${data.room_name ? 'Кладовка ' + data.room_name : ''}</h5>
=======
...
<<<<<<< HEAD
  <button class="button btn-rent-room btn-modal-payments-room-open link-rent-storage-room" data-room-id="${data.room_id &&
    data.room_id}">
    <span>Арендовать</span>
  </button>
                  ${isReplace
      ? `<button class="button btn-replace-room" data-room-id="${data.room_id &&
      data.room_id}">
                    <span>Заменить</span>
                  </button>`
      : ''
...
export function roomModalHtml(data) {
  return `
<<<<<<< HEAD
  <div class="warehouse__rooms_room room-modal" data-room-id="${data.room_id &&
    data.room_id}">
              <div class="warehouse__rooms_room-content">
                <span class="warehouse__rooms_room-num">${data.room_name &&
    '№' + data.room_name}</span>
                <span class="warehouse__rooms_room-area">${data.area ? data.area + ' м²' : ''
    }</span>
=======
...
<<<<<<< HEAD
              <button class="warehouse__rooms_room-link button btn-replace-room-modal btn-modal-close" data-room-id="${data.room_id &&
    data.room_id}">
                <span>Заменить</span>
              </button>
            </div>`
