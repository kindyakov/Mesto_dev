...
class AccessStorage {
	constructor() {
		...
		this.loader = new Loader(document.querySelector('.main'), {
			...
			position: 'fixed',
			id: 'assets-storage-loader',
		})
		this.storeroomsSliderWrapper = document.querySelector(
		...
		this.SelectWarehouse = new SelectWarehouse({ loader: this.loader })
	...
	events() {
		this.assetsStorage.addEventListener('click', e => {
			if (e.target.closest('.btn-open-room')) {
				this.SelectWarehouse.nextModal = warehouse =>
					this.openRoom.open(warehouse, this.clientData)
				this.SelectWarehouse.open(this.clientData)
			}
			if (e.target.closest('.btn-open-barrier')) {
				this.SelectWarehouse.nextModal = warehouse =>
					this.openBarrier.open(warehouse)
				this.SelectWarehouse.open(this.clientData)
			}
			if (e.target.closest('.btn-open-gates')) {
				this.SelectWarehouse.nextModal = warehouse =>
					this.openGates.open(warehouse)
				this.SelectWarehouse.open(this.clientData)
			}
			if (e.target.closest('.btn-open-door')) {
				this.SelectWarehouse.nextModal = warehouse =>
					this.openDoor.open(warehouse)
				this.SelectWarehouse.open(this.clientData)
			}
		})
	...
	render({ accountTabs, clientTotalData }) {
		...
		if (!rooms.length && !test_rooms.length) return
		this.storeroomsSliderWrapper.innerHTML = [...rooms, ...test_rooms].map(room => rooms2Html(room)).join('')
	}
}
