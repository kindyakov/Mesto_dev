...
export function storageRoomHtml(data) {
  return `<div class="txt-content-product" data-room-id="${data.room_id ? data.room_id : ''
                    ...
                    <div class="top-row">
                      <p class="title">${data.room_name ? 'Кладовка ' + data.room_name : ''
    }</p>
      ...
      : ''
                    ...
                    <div class="container-attributes">
                      <div class="row">
                        ...
                        <p class="value-attribute">${data.area ? data.area + ' м²' : ''
    }</p>
                      </div>
                      <div class="row">
                        <p class="title-attribute">Объём</p>
                        <p class="value-attribute">${data.volume ? data.volume + ' м³' : ''
    }</p>
                      </div>
                      <div class="row">
                        <p class="title-attribute">Размер кладовки (ДхШхВ)</p>
                        <p class="value-attribute">${data.dimensions ? data.dimensions : ''
    }</p>
                      </div>
                    </div>
                  </div>`
...
export function rooms2Html(data) {
  ...
  <div class="agreement2__container-bottom__row__block room-bottom account-storerooms-rooms__slider-slide swiper-slide" data-room-id="${data.room_id ? data.room_id : ''
                ...
                <div class="agreement2__container-bottom__row__block__row-1 account-storerooms-rooms__slider-slide_top">
                  <h4 style="line-height: 70%;">${data.room_name ? 'Кладовка ' + data.room_name + `<br><span style="font-size: 14px;">${data.warehouse_name}</span>` : ''}</h4>
                  <p>
      ...
      : ''
                ...
                ${data.rented === 0.75
      ? `<button class="button btn-open-room-modal btn-modal-close" data-room-id="${data.room_id ? data.room_id : ''
      }"><span>Открыть</span></button>`
      : `<button class="btn-payment btn-modal-payments-room-open button" data-room-id="${data.room_id ? data.room_id : ''
  ...
  <div class="modal-select-warehouse__warehouse btn-modal-close" data-warehouse-id="${data.warehouse_id ? data.warehouse_id : ''
            ...
            <div class="modal-select-warehouse__warehouse_content">
              ...
              <address>
                ...
                <span>${data.warehouse_address ? data.warehouse_address : ''
    }</span>
              </address>
            </div>
          </div>`
