...
class WarehousesResult {
	...
	init() {
		...
		this.wrapper.insertAdjacentHTML(
			'beforeend',
			`<div class="warehouses-result"></div>`
		)
	...
	render(warehouse, rangeData) {
		...
		if (warehouse.warehouses?.length) {
			warehouse.warehouses.forEach(_warehouse => {
				...
				this.warehousesResult.insertAdjacentHTML(
					'beforeend',
					warehousesResultAccordion({
						data: _warehouse,
						isRooms: false,
						rangeData,
					})
			...
			setMinMaxBlocks('.warehouses-result__accordion_name-room', {
				breakpoints: [992],
			})
		} else {
			if (warehouse.rooms?.length) {
				this.warehousesResult.insertAdjacentHTML(
					'beforeend',
					warehousesResultAccordion({ data: warehouse, rangeData })
				)
				setMinMaxBlocks('.warehouses-result__accordion_name-room', {
					breakpoints: [992],
				})
				setMinMaxBlocks('.warehouses-result__accordion_content-room .area', {
					breakpoints: [992],
				})
				setMinMaxBlocks('.warehouses-result__accordion_content-room .volume', {
					breakpoints: [992],
				})
				setMinMaxBlocks(
					'.warehouses-result__accordion_content-room .dimensions',
					{ breakpoints: [992] }
				)
				setMinMaxBlocks('.warehouses-result__accordion_content-room .price', {
					breakpoints: [992],
				})
				this.accordion.init()
			} else {
		...
		if (warehouse.rooms?.length || warehouse.warehouses?.length) {
			if (!isAnim) {
				this.warehousesResult
					.animate(
						[{ maxHeight: 0 }, { maxHeight: heightWarehousesResult + 'px' }],
						{ duration: 300, easing: 'ease-in-out' }
					)
					.addEventListener('finish', () => {
						this.warehousesResult.scrollIntoView({
							behavior: 'smooth',
							block: 'center',
						})
						this.accordion?.open(
							this.warehousesResult.querySelector('._my-accordion'),
							this.warehousesResult.querySelector('._my-accordion-content')
						)
					})
			} else {
				this.warehousesResult.scrollIntoView({
