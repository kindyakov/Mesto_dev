...
export function warehousesResultAccordion({ data, isRooms = true, rangeData }) {
  ...
  <div class="warehouses-result__accordion _my-accordion ${
		...
		data.warehouse_id
	}${queryParams}" class="warehouses-result__accordion_button button-2" data-warehouse-id="${
		data.warehouse_id ? data.warehouse_id : ''
	}"><span>Выбрать кладовку</span></a>
		${
			isRooms
				? `
      ...
      ${
				data.rooms?.length
					? data.rooms.map(room => warehousesResultRoom(room)).join('')
					: ''
			}
    </div>`
				: ''
...
export function warehousesResultRoom(data) {
	return `
  <div class="warehouses-result__accordion_room" data-room-id="${data.room_id}">
		...
		data.room_id
			...
			<svg class='icon icon-more'>
				<use xlink:href='${window.app.directory}/assets/img/svg/sprite.svg#more'></use>
			</svg>
		</a>
	</div>`
}
