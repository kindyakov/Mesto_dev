...
class RentRoom {
	constructor() {
		...
		this.authorizationTabs = new Tabs('authorization-rent-room-tabs', {
			activeIndexTab: 1,
		})
		this.authorization = new Authorization({
			wrapperSelector: '.authorization-rent-room',
		})
		...
		this.formPaymentInvoice = this.rentRoom.querySelector(
			'.form-payment-invoice'
		)
		this.validatorPaymentInvoice = validatePaymentInvoice(
			this.formPaymentInvoice
		)

		this.formAgreementConclusion = this.rentRoom.querySelector(
			'.form-agreement-conclusion'
		)
		this.validatorAgreementConclusion = validateAgreementConclusion(
			this.formAgreementConclusion
		)
	...
	initModals() {
		this.modalCheckingDetailsForm = new Modal('.modal-checking-details-form', {
	...
	initDatePickers() {
		...
		if (this.inputDate) {
			this.inputDate.value = getFormattedDate(new Date(), 'DD-MM-YYYY')
			this.agrBegDatePicker = new AirDatepicker(this.inputDate, {
				...
				onSelect: ({ date, datepicker }) => {
					...
					this.agrBegDate = getFormattedDate(date, 'YYYY-MM-DD')
				},
			})

			if (!this.agrBegDate) {
				this.agrBegDate = getFormattedDate(new Date(), 'YYYY-MM-DD')
			}
		}
	}
	...
	events() {
		...
		document.addEventListener('click', e => {
			if (e.target.closest('.tab-registration')) {
				e.preventDefault()
				this.authorizationTabs.switchTabs(
					this.authorizationTabs.tabs.querySelector('.btn-registration')
				)
			}
			...
			if (e.target.closest('.tab-authorization')) {
				e.preventDefault()
				this.authorizationTabs.switchTabs(
					this.authorizationTabs.tabs.querySelector('.btn-authorization')
				)
			}
			...
			if (e.target.closest('.item-month')) {
				...
				const itemMonthActive = this.rentRoom.querySelector(
					'.item-month._active'
				)
				if (!month) return

				const [price, deposit] = this.calculatePriceAndDepositByMonth(
					month,
					this.agreementData
				)
			...
			if (e.target.closest('.button-send-form-payment-invoice')) {
				this.formDataPaymentInvoice &&
					sendFormAgreementInvoice(
						this.formDataPaymentInvoice,
						this.loader
					).then(data => {
						if (!data) return
						this.modalCheckingInfoForm.modal
							.querySelector('.btn-download-invoice')
							?.setAttribute('data-bill-id', data.bill_id)
						this.modalCheckingInfoForm.modal.querySelector(
							'.content'
						).innerHTML = formInfoModal({ ...data, email: this.user.email })
		...
		this.inputMonth &&
			this.inputMonth.addEventListener('input', e => {
				...
				const itemMonthCurrent = this.rentRoom.querySelector(
					`.item-month[data-month="${this.inputMonth.value}"]`
				)
				...
				if (
					(+value.slice(0, 1) !== 1 && this.inputMonth.value.length === 1) ||
					this.inputMonth.value.length > 1
				) {
					e.target.blur()
					this.payment.scrollIntoView({ behavior: 'smooth', block: 'start' })
				}
		...
		this.formPaymentOnline &&
			this.formPaymentOnline.addEventListener(
				'submit',
				this.submitPaymentOnline.bind(this)
			)
		this.formPaymentInvoice &&
			this.formPaymentInvoice.addEventListener(
				'submit',
				this.submitPaymentInvoice.bind(this)
			)
		this.formAgreementConclusion &&
			this.formAgreementConclusion.addEventListener(
				'submit',
	...
	init() {
		...
		if (this.isAuth) {
			this.activeIndexStep = this.urlParams.get('step')
				? +this.urlParams.get('step')
				: 1
		...
		if (this.roomIds.length) {
			this.renderRooms(
				buildQueryParams({ room_ids: JSON.stringify(this.roomIds) })
			)
		}
	}
	...
	swapStep(index) {
		...
		const activeStepProgress = this.rentRoom.querySelector(
			'.step-progress._active'
		)
		const activeStep = this.rentRoom.querySelector(
			'.rent-room-step._step-active'
		)
	...
	submitPaymentInvoice(e) {
		...
		if (this.validatorPaymentInvoice.isValid) {
			...
			this.modalCheckingDetailsForm.modal.querySelector(
				'.content'
			).innerHTML = formDetailsModal(data)
			this.modalCheckingDetailsForm.open()
		} else {
			this.scrollToErrInput(
				this.formPaymentInvoice.querySelector('.just-validate-error-field')
			)
		}
	}
	...
	submitAgreementConclusion(e) {
		...
		if (this.validatorAgreementConclusion.isValid) {
			...
			this.scrollToErrInput(
				this.formAgreementConclusion.querySelector(
	...
	async renderRooms(queryParams) {
		try {
			...
			const bottomRoomsSlider = this.rentRoom.querySelector(
				'.bottom-rooms-slider-rent-room'
			)
	...
	async getUser() {
		try {
			...
			if (response.status !== 200) return
			const inputsAgreementConclusion = this.rentRoom.querySelectorAll(
				'.input-agreement-conclusion'
			)
			const inputsU = this.rentRoom.querySelectorAll(
				'.rent-room__payment_input'
			)
			const { user } = response.data
			this.user = user

			inputsAgreementConclusion.length &&
				inputsAgreementConclusion.forEach(input => {
					input.value = user[input.name] ? user[input.name] : ''
				})

			inputsU.length &&
				inputsU.forEach(input => {
					input.value = user[input.name] ? user[input.name] : ''
				})

			if (user.user_type === 'f') {
				document
					.querySelector('.rent-room__payment_tabs')
					.classList.add('_none')
				this.paymentTabs.switchTabs(
					document.querySelector('[data-tabs-btn="payment-rent-room-tabs-0"]')
				)
				...
				document
					...
					.classList.add('_none')
				this.paymentTabs.switchTabs(
					document.querySelector('[data-tabs-btn="payment-rent-room-tabs-1"]')
				)
			} else {
				document
					.querySelector('.rent-room__payment_tabs')
					.classList.remove('_none')
			}
	...
	changePriceSum(priceSum, deposit) {
		const infoPaymentRentRoom = this.rentRoom.querySelector(
			'.info-payment-rent-room'
		)
		infoPaymentRentRoom.innerHTML = `
    <span>Арендный платеж за первый месяц</span>
    <b style="text-align: right;">${formattingPrice(+priceSum)}</b>
    <span>Депозит (вернется после окончания аренды)</span>
    <b style="text-align: right;">${formattingPrice(+deposit)}</b>`

		this.resultPriceRooms.length &&
			this.resultPriceRooms.forEach((el, i) => {
				el.textContent = formattingPrice(+priceSum + +deposit)
			})
	}
}
