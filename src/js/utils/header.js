...
export const scroll = (
	...
	function scrollToSection(id) {
		...
		const topPos =
			...
			(headerBody && isHeader ? headerBody.clientHeight + 20 : 0)
...
export const dropdown = ({
	...
	function handleClick(e) {
		...
		const dropdown = btn.parentElement.querySelector(
			dropdownLevel === '1' ? selectorDropdown : selectorSubDropdown
		)

		if (btn.classList.contains(selectorDropdownBtnActive)) {
			const dropdownsSecondLevel = dropdown.querySelectorAll(
				selectorSubDropdown
			)
			const dropdownButtonsSecondLevel = dropdown.querySelectorAll(
				selectorDropdownBtn
			)

			dropdownsSecondLevel.length &&
				dropdownsSecondLevel.forEach((dropdownSecondLevel, i) => {
					dropdownButtonsSecondLevel[i].classList.remove(
						selectorDropdownBtnActive
					)
			...
			if (dropdownLevel === '2') {
				...
				dropdownFirstLevel.style.maxHeight =
					dropdownFirstLevel.scrollHeight + dropdown.scrollHeight + 30 + 'px' // 30 - это отступы которые добавляются с классом selectorDropdown у selectorSubDropdown
	...
	function handleMediaChange(mediaQueryList) {
		if (mediaQueryList.matches) {
			...
			const dropdownsSecondLevel = document.querySelectorAll(
				selectorSubDropdown
			)
...
export function updateLinks(linkSelector = '.link') {
...
export const scroll = (selectorLinks = '.link-scroll', { isHeader = true } = {}) => {
  const links = document.querySelectorAll(selectorLinks)
  const headerBody = document.querySelector('.header__body')
  const fullPageHeight = Math.max(
    document.body.scrollHeight, // Высота всего документа включая прокрутку
