...
<<<<<<< HEAD
	constructor(selector, options) {
		...
		this.activeIndexTab = this.options.activeIndexTab
	...
	check() {
		if (
			document.querySelectorAll(`[data-tabs-init="${this.selector}"]`).length >
			1
		) {
			console.error(
				'Количество элементов с одинаковым data-tabs-init больше одного!'
			)
			return false
		}

		if (
			this.tabsBtns.length !== this.tabsContents.length &&
			!this.options.lotOfTabs
		) {
		...
		if (this.options.lotOfTabs && this.options.lotOfTabs.length) {
			...
			tabsWp.length &&
				tabsWp.forEach((tabWp, i) => {
					...
					tabsContent.length &&
						tabsContent.forEach((content, _i) => {
							if (_i === this.options.activeIndexTab) {
			...
			this.tabsContents.length &&
				this.tabsContents.forEach((el, i) => {
					el.classList.remove('_tab-content-active')
					el.setAttribute(
						'data-tabs-content',
						this.tabsBtns[i].getAttribute('data-tabs-btn')
					)
				})
		}

		if (!this.tabsBtns.length || !this.tabsContents.length) return

		this.tabsBtns[this.options.activeIndexTab].classList.add('_tab-btn-active')
		this.tabsContents[this.options.activeIndexTab].classList.add(
			'_tab-content-active'
		)
	...
	events() {
		this.tabsBtns.forEach((el, i) => {
			el.addEventListener('click', e => {
				...
				if (this.options.wrapperTabBtnSelector) {
					const wrapperTabBtn = this.tabs.querySelector(
						this.options.wrapperTabBtnSelector
					)
					prevTabBtn = wrapperTabBtn.querySelector(
						`${this.options.btnSelector}._tab-btn-active`
					)
				} else {
					prevTabBtn = el.parentElement.querySelector(
						`${this.options.btnSelector}._tab-btn-active`
					)
				}
	...
	switchTabs(
		nexTabBtn,
		prevTabBtn = this.tabs.querySelector(
			`${this.options.btnSelector}._tab-btn-active`
		)
	) {
		...
		const prevTabContent = this.tabs.querySelector(
			`${this.options.contentSelector}[data-tabs-content="${prevTabId}"]`
		)
		const nextTabContent = this.tabs.querySelector(
			`${this.options.contentSelector}[data-tabs-content="${nextTabId}"]`
		)
		...
		this.activeIndexTab = Array.from(this.tabsBtns).indexOf(nexTabBtn)
  ...
  events() {
    this.tabsBtns.forEach((el, i) => {
      el.addEventListener('click', e => {
        ...
        if (this.options.wrapperTabBtnSelector) {
          const wrapperTabBtn = this.tabs.querySelector(this.options.wrapperTabBtnSelector)
          prevTabBtn = wrapperTabBtn.querySelector(`${this.options.btnSelector}._tab-btn-active`)
        } else {
          prevTabBtn = el.parentElement.querySelector(`${this.options.btnSelector}._tab-btn-active`)
        }
